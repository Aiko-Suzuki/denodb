...
export class SQLTranslator implements Translator {
	...
	translateToQuery(query: QueryDescription): Query {
		let queryBuilder = new (SQLQueryBuilder as any)({
			...
			useNullAsDefault: this._dialect === "surreal",
			log: {
		...
		switch (query.type) {
			case "drop":
				const dropTableHelper = query.ifExists ? "dropTableIfExists" : "dropTable";
			...
			case "create":
				...
				const createTableHelper = query.ifExists ? "createTable" : "createTableIfNotExists";

				queryBuilder = queryBuilder.schema[createTableHelper](query.table, (table: any) => {
	...
	formatFieldNameToDatabase(fieldName: string | FieldAlias): string | FieldAlias {
		if (typeof fieldName === "string") {
			const dotIndex = fieldName.indexOf(".");

			// Table.fieldName
			if (dotIndex !== -1) {
				return fieldName.slice(0, dotIndex + 1) + snakeCase(fieldName.slice(dotIndex + 1));
			}

			return snakeCase(fieldName);
		} else {
			return Object.entries(fieldName).reduce((prev: any, [alias, fullName]) => {
